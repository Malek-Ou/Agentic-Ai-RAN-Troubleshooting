"""
Étape 3 — Calcul du Score Hybride et injection Neo4j
=====================================================

Formule :
    score(RC | KPI) = W_DOC × P_doc(KPI | RC)  +  W_PRIOR × P_prior(RC)
    Après normalisation par KPI : Σ posterior_prob = 1.0

Sources :
    P_doc   ← build_pdoc_table_v3.py  (extraction auto sur 35 PDFs Huawei)
    P_prior ← ingest_oss_alarms.py    (fréquences alarmes OSS terrain)

Schéma Neo4j :
    (:KPICategory)-[:HAS_ROOT_CAUSE {posterior_prob, p_doc, p_prior,
                    confidence, source, doc_ref, top_evidence}]->(:RootCause)

Pré-requis :
    → ingest_kg.py           doit avoir tourné  (:KPICategory nodes)
    → ingest_oss_alarms.py   doit avoir tourné  (:RootCause nodes avec rc.id)
    → build_pdoc_table_v3.py doit avoir tourné  (pdoc_auto.py généré)

CORRECTIONS v3.0
-----------------
  FIX-1 : POSTERIOR_THRESHOLD = 0.05 (après normalisation sur 8 RC)
           Avant 0.25 puis 0.15 : appliqué sur le score BRUT, non normalisé.
           Résultat : avec 8 RC, score normalisé moyen = 1/8 = 0.125 → la
           plupart des RC filtrées → 0 à 4 relations injectées au lieu de ~30.
           Avec 0.05 post-normalisation : toutes les paires documentées passent.

  FIX-2 : Paires absentes de P_DOC_TABLE → skip (revert du "floor p_doc").
           Le floor p_doc gonflait le dénominateur de normalisation avec des
           entrées sans valeur documentaire → dilution des vrais scores.

  FIX-3 : Cypher MATCH sur :KPICategory (label réel créé par ingest_kg.py)
           et rc.id (clé stable créée par ingest_oss_alarms.py via to_id()).
           Avant : MATCH (k:KPI {category:...}) → aucun nœud trouvé car
           ingest_kg.py crée :KPICategory, pas :KPI avec category.

  FIX-4 : Entropie supprimée — ni stockée en Neo4j, ni calculée, ni logguée.

  FIX-5 : POSTERIOR_THRESHOLD appliqué AVANT normalisation sur le score brut.
           Filtre le vrai bruit (paires non documentées avec prior très faible)
           sans biaiser la distribution normalisée des paires documentées.
"""
